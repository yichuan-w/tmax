...
#   CLASSIFY_ONLY_SOLVED=1  Only classify tasks that have a solutions summary
#                    (the subset the comparison scores). Use for very large
#                    baselines like SWE-smith (~59k dirs, 250 solved) so you
#                    don't pay to classify tens of thousands of unscored tasks.
...
CLASSIFY_ONLY_SOLVED="${CLASSIFY_ONLY_SOLVED:-0}"
...
for d in $CLASSIFY_DIRS; do
  ...
  if [[ "$CLASSIFY_ONLY_SOLVED" == "1" ]]; then
    ARGS+=(--only-solved)
  fi
